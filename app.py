...
# Logo & 標題
st.image("https://sites.google.com/view/trumi-jewelry/_/rsrc/1688543987023/home/trumi-logo.png", width=180)
...
def 批次匯入進貨(df: pd.DataFrame) -> int:
    ...
    for _, r in df.iterrows():
        ...
        sid = 取得對映('細項')[r['細項']]
        date_str = r.get('日期') or datetime.now().strftime('%Y-%m-%d')
        新增('進貨', ['類別編號','品項編號','細項編號','數量','單價','日期'],
             [cid,iid,sid,int(r['買入數量']),float(r['買入單價']), date_str])
        cnt += 1
    return cnt
...
def 批次匯入銷售(df: pd.DataFrame) -> int:
    ...
    for _, r in df.iterrows():
        ...
        新增('銷售', ['類別編號','品項編號','細項編號','數量','單價','日期'],
             [cid,iid,sid,int(r['賣出數量']),float(r['賣出單價']), date_str])
        cnt += 1
    return cnt

# --- 側邊欄導覽 ---
section = st.sidebar.selectbox("瀏覽 Tru-Mi 官網內容", [
    "🔥 熱銷推薦", "🎨 主題系列", "🤍 婚戒物語", "💌 訂製服務", "🔧 庫存系統"
])
if section != "🔧 庫存系統":
    st.header(section)
    st.info("此區功能開發中，請切換到「庫存系統」使用管理介面。")
    st.stop()
...
if menu == '類別管理':
    ...
    with tab2:
        ...
        if not cmap:
            ...
            sel_item = st.selectbox('選擇編輯的品項', list(series_map.keys()), key='series_sel')
    ...
    with tab2:
        ...
        if not cmap: st.warning('請先新增類別')
            ...
            df_i=pd.read_sql('SELECT 品項編號,品項名稱 FROM 品項 WHERE 類別編號=?',conn,params=(cid,))
            imap=dict(zip(df_i['品項名稱'],df_i['品項編號']))
            if not imap: st.warning('該類別無品項')
            else:
                sel2=st.selectbox('品項', list(imap.keys())); iid=imap[sel2]
                df_s=pd.read_sql('SELECT 細項編號,細項名稱,圖片 FROM 細項 WHERE 品項編號=?',conn,params=(iid,))\
                    .rename(columns={'細項編號':'編號','細項名稱':'名稱'})
                st.table(df_s)
                # 圖片上傳 / 預覽
                sid_map = dict(zip(df_s['名稱'], df_s['編號']))
                sel_sub = st.selectbox('選擇細項', list(sid_map.keys()), key='img_sel'); sid = sid_map[sel_sub]
                img_path = df_s[df_s['編號']==sid]['圖片'].iloc[0]
                ...
                # CSV下載 & 單筆 CRUD
                st.download_button('下載此品項細項 CSV',
    ...
    with tab1:
        sample = pd.DataFrame({
            ...
            '買入數量':[10],'買入單價':[100.0],'日期':[date.today().strftime('%Y-%m-%d')]
        })
        st.download_button('下載進貨批次範例',
        ...
        if up:
            ...
            cnt = 批次匯入進貨(df); st.success(f'批次匯入 {cnt} 筆進貨紀錄')
    ...
    with tab4:
        ...
        if dfp.empty:
            ...
            sel    = st.selectbox('選擇紀錄', list(desc_map.keys()), key='edit_p_sel')
            rid    = desc_map[sel]
            row    = dfp[dfp['紀錄ID']==rid].iloc[0]
            date_n = st.date_input('日期', value=pd.to_datetime(row['日期']).date(), key='edit_p_date')
            qty_n  = st.number_input('數量', min_value=1, value=int(row['數量']), key='edit_p_qty')
            price_n= st.number_input('單價', min_value=0.0, format='%.2f', value=float(row['單價']), key='edit_p_price')
            if st.button('更新進貨', key='edit_p_save'):
                total = qty_n * price_n
                c.execute(
                    'UPDATE 進貨 SET 數量=?,單價=?,總價=?,日期=? WHERE 紀錄ID=?',
                    (qty_n, price_n, total, date_n.strftime('%Y-%m-%d'), rid)
                )
