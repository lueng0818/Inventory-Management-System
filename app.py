...
# 進貨管理
 ...
 # 編輯/刪除銷售紀錄
    with tab3:
        ...
        # 選擇要操作的紀錄
        rec = st.number_input('輸入紀錄ID', min_value=1, step=1, key='sell_rec')
        rec = int(rec)
        ...
        update_date = st.checkbox('更新日期', key='sell_update_date')
        nd = st.date_input(
            '新日期',
        ...
        # 更新按鈕
        if st.button('更新銷售紀錄', key='btn_update_sell'):
        ...
        # 刪除按鈕
        if st.button('刪除銷售紀錄', key='btn_delete_sell'):
            刪除('銷售', '紀錄ID', rec)
