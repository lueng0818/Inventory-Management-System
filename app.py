...
def 取得對映(table, key, val):
    ...
    st.warning(f"在 {table} 表中找不到 '{key}' 或 '{val}' 欄位")
    return {}
...
def 批次匯入進貨(df):
    ...
    for _, row in df.iterrows():
        ...
        df_i = pd.read_sql('SELECT 品項編號 FROM 品項 WHERE 類別編號=? AND 品項名稱=?', conn, params=(cid,item))
        iid = df_i['品項編號'].iloc[0]
        新增('細項',['品項編號','細項名稱'],[iid,sub])
        df_su = pd.read_sql('SELECT 細項編號 FROM 細項 WHERE 品項編號=? AND 細項名稱=?', conn, params=(iid,sub))
