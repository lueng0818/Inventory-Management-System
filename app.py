...
def 新增(table: str, cols: list, vals: list) -> int:
    ...
    try:
        ...
        conn.commit()
        new_id = c.lastrowid
        st.session_state['last_action'] = {'type':'新增','table':table,'pk_col':cols_used[0],'pk_val':new_id}
        return new_id
...
if menu == '類別管理':
    ...
    st.table(df)
    # 匯出按鈕
    st.download_button('下載類別 CSV', df.to_csv(index=False, encoding='utf-8-sig'), 'categories.csv', 'text/csv')
...
# 品項管理
    ...
    if not cmap:
        ...
        st.table(df)
        st.download_button('下載品項 CSV', df.to_csv(index=False, encoding='utf-8-sig'), f'items_cat{cid}.csv', 'text/csv')
...
# 細項管理
    ...
    if not cmap:
        ...
        if not imap:
            ...
            st.table(df_s)
            st.download_button('下載細項 CSV', df_s.to_csv(index=False, encoding='utf-8-sig'), f'subs_item{iid}.csv', 'text/csv')
...
# 進貨管理
    ...
    with tab1:
        df = 查詢('進貨')
        st.download_button('下載所有進貨記錄 CSV', df.to_csv(index=False, encoding='utf-8-sig'), 'purchases.csv', 'text/csv')
    with tab2:
        # ... 省略手動記錄邏輯，保持現有 ...
        pass
...
# 銷售管理
    ...
    with tab1:
        df = 查詢('銷售')
        st.download_button('下載所有銷售記錄 CSV', df.to_csv(index=False, encoding='utf-8-sig'), 'sales.csv', 'text/csv')
    with tab2:
        # ... 省略手動記錄邏輯，保持現有 ...
        pass
...
# 儀表板
    ...
    st.download_button('下載庫存摘要 CSV', summary.to_csv(index=False, encoding='utf-8-sig'), 'summary.csv', 'text/csv')
